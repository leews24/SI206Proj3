...
programmers = [Student(tups[0],tups[1],tups[2]) for tups in student_tups_list]
...
names_with_not_too_much_seniority = [student.name for student in list(programmers) if len(student.name) > student.years_UM]
...
def readfiles(fnamelist):
    for files in fnamelist:
        fvar = open(files, 'r')
        templist = fvar.readlines()
        for lines in templist:
            yield lines
        fvar.close()

# Define len_check
def len_check(linegen):
    for line in linegen:
        if len(line) > 40:
            yield line

# Define main_filterer
def main_filterer(fnamelist):
    return len_check(readfiles(fnamelist))
